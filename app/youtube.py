...
def get_video_links(session: web.SimpleSession, youtube_home_url: str, youtube_search_url: str, youtube_query_payload: dict):

    # First, enter the search form on the youtube home page
    response = session.enter_search_form(search_url=youtube_search_url, payload=youtube_query_payload)
    if response == None:
        logger.error(f"Error occurred performing a search for {youtube_query} against url {youtube_search_url}. Please try again.")
        return None
    # Get the list of hrefs to each video on the home page
    links = response.html.find("#video-title")
    if len(links) == 0:
        logger.warn(f"Thats odd. Youtube gave me no videos for this search. Youll have to try something else: {youtube_query_payload}")
        return
    link_list = []
    ...
    for idx, link in enumerate(links):
        if 'title' in link.attrs and 'href' in link.attrs:
            link_list.append(f"{link.attrs['title']} - {youtube_home_url+link.attrs['href']}")
    ...
    video_selection_idx = common.select_items_from_list("Select the song you wish to download!", link_list, 1, return_value=False)
...
def run_download_process(session: web.SimpleSession, file_path_no_format: str, youtube_home_url: str, youtube_search_url: str, youtube_query_payload: dict, file_format: str) -> str:
    ...
    video_url = get_video_links(session, youtube_home_url, youtube_search_url, youtube_query_payload)
