...
def itunes_lib_search(itunes_lib_path: str, search_parameters: str, album_properties=Optional[itunes_api.ItunesApiAlbumKeys]) -> List[str]:
    """
    ...
    for song_path in itunes_songs:
        ...
        song_name = remove_illegal_characters(song_name_split[len(song_name_split)-1].lower())
        album_name = remove_illegal_characters(song_name_split[len(song_name_split)-2].lower())
        artist_name = remove_illegal_characters(song_name_split[len(song_name_split)-3].lower())
        ...
        if album_properties == None:
            ...
            if album_properties.artistName.lower() in artist_name and search_parameters in song_name:
                matches.append(song_path)
...
def m4a_tagger(file_path: str, song_tag_data: itunes_api.ItunesApiSongModel) -> bool:
    ...
    try:
        ...
        if song_tag_data.collectionArtistName is not None: # check if collection_artist_name exists before adding to tags
...
def parse_itunes_search_api(search_variable: str, mode: modes.Modes, limit:int=20, lookup: bool=False) -> itunes_api.ItunesApiSongModel:
    parsed_results_list = query_api(search_variable, limit, mode,
                                  lookup=lookup)
...
def launch_album_mode(artist_album_string=''):
    ...
    """
    while True:
        album_props = parse_itunes_search_api(search_variable=artist_album_string,
        ...
        songs_in_album_props = query_api(search_variable=album_props.collectionId, # get list of songs for chosen album
                                                    limit=album_props.trackCount, mode=modes.Modes.SONG,
                                                    lookup=True)
...
def query_api(search_variable: str, limit: int, mode: modes.Modes, lookup: bool=False) -> List[Union[itunes_api.ItunesApiSongModel, itunes_api.ItunesApiAlbumKeys]]:
    """
    params:
    ...
    if not lookup: # perform general search
        search_parameters = {'term':search_variable, 'entity':mode.value, 'limit':limit}
        ...
        search_parameters = {'id':search_variable, 'entity':mode.value, 'limit':limit}
